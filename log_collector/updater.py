...
def restart_application():
    ...
    """
    # Extract the original command without any arguments
    cmd = sys.argv[0]
    
    # Skip any existing arguments - just restart with the base command
    logger.info(f"Restarting with command: {cmd}")
    
    # This approach works for both script (.py) and installed executables
    try:
        if os.name == 'nt':  # Windows
            # On Windows, we need to use subprocess to start a new process
            # and then exit the current one
            import subprocess
            subprocess.Popen([cmd])
            sys.exit(0)
        else:  # Unix-like
            # Use execv which replaces the current process
            os.execv(cmd, [cmd])
    except Exception as e:
        logger.error(f"Failed to restart application: {e}")
        print(f"{Fore.RED}Failed to restart automatically. Please restart the application manually.{ColorStyle.RESET_ALL}")
        sys.exit(0)
